...
def save_to_markdown(params: dict) -> str:
    """Save book sections to a Markdown file and return the filename."""
    sections: List[BookSection] = [BookSection(**section) for section in params.get("sections", [])]
...
try:
    save_markdown_tool = Tool(
        name="SaveToMarkdown",
        description="Saves a list of book sections to a Markdown file.",
                    ...
                    "properties": {
                        "sections": {
                            ...
                            "items": {
                                "type": "object",
                                "properties": {
                                    "title": {"type": "string", "description": "Title of the book section"},
                                    "summary": {"type": "string", "description": "Detailed summary of the section"},
                                    "key_concepts": {
                                        "type": "array",
                                        "items": {"type": "string"},
                                        "description": "List of key concepts or keywords"
                                    },
                                    "code_examples": {
                                        "type": "array",
                                        "items": {"type": "string"},
                                    ...
                                    "explanation": {"type": "string", "description": "Explanation of the concepts or code"}
                                },
                                "required": ["title", "summary", "key_concepts", "code_examples", "explanation"]
                            }
                        },
                        "video_id": {"type": "string", "description": "The YouTube video ID to include in the filename"}
                    },
                    "required": ["sections", "video_id"]
                }
            }
        }
    )
    logger.info("SaveToMarkdown tool initialized successfully")
except Exception as e:
    logger.error(f"Failed to initialize SaveToMarkdown tool: {e}")
    raise
...
"""
        ...
        with trace("Saving book sections to Markdown"):
            filename = save_to_markdown({"sections": [section.model_dump() for section in sections], "video_id": video_id})
            if filename:
