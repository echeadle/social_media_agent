...
import sys
import asyncio
from typing import List, Sequence, TypedDict, Literal, Optional
import os
...
from utils import get_youtube_video_id, get_transcript, get_youtube_video_title
...
def save_to_markdown(sections: List[BookSection], video_id: str, video_name: Optional[str]) -> str:
    """
    ...
    Args:
        ...
        video_name: The name of the YouTube video, if available.
    ...
    if video_name:
        markdown_content += f"Video Name: {video_name}\n"
...
async def main():
    ...
    try:
        ...
        video_name = get_youtube_video_title(video_id)
        if video_name:
            logger.info(f"Video Name: {video_name}")
        else:
            logger.warning(f"Could not retrieve video name for ID: {video_id}. Proceeding without video name.")
...
"""
        ...
        with trace("Saving book sections to Markdown"):
            result_message = save_to_markdown(sections=sections, video_id=video_id, video_name=video_name)
